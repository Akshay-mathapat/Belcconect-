...
export async function GET(
  ...
  try {
    ...
    if (!isCustomer && !isProvider) {
      return NextResponse.json(
        { error: "Forbidden: You are not authorized to view reviews for this booking" },
        { status: 403 }
      );
    }
...
export async function POST(
  ...
  try {
    ...
    // 3. Participation check: Must be customer or provider of this booking
    const isCustomer = authUser.userId === booking.customer_id;
    const isProvider = authUser.userId === booking.provider_id;
    ...
    // 4. Lifecycle check: Reviews are ONLY allowed when booking is Completed (or legacy ReviewSubmitted)
    const allowedStatuses = ["Completed", "ReviewSubmitted"];
    if (!allowedStatuses.includes(booking.status)) {
      return NextResponse.json(
        { error: `Reviews can only be submitted after the service is Completed (current status: ${booking.status})` },
        { status: 400 }
      );
    }
    ...
    // 6. Identity derivation: NEVER trust client-supplied reviewerId or revieweeId
    ...
    const revieweeRole = isCustomer ? "provider" : "customer";

    if (!revieweeId) {
      return NextResponse.json(
        { error: "Incomplete booking record: missing reviewee identity" },
        { status: 400 }
      );
    }

    // 7. Duplicate prevention: Check if reviewer already reviewed this direction
    ...
    // 8. Insert into booking_reviews
    const reviewId = `rev-${Date.now()}-${Math.floor(Math.random() * 10000)}`;
    await query(
      `INSERT INTO booking_reviews (
        id, booking_id, reviewer_id, reviewer_role, reviewee_id, reviewee_role, rating, comment
    ...
    // 9. Backwards compatibility: If Customer reviewed Provider, backfill legacy bookings.rating and bookings.review_comment
    // Note: Completed status remains Completed (does NOT rewrite status)
    if (isCustomer) {
      await query(
        `UPDATE bookings
         SET rating = $1, review_comment = $2
         WHERE id = $3`,
        [parsedRating, cleanComment || null, canonicalBookingId]
      );
    }
    ...
    console.error(`Error creating review for booking ${cleanId}:`, error);
    return NextResponse.json({ error: error.message || "Internal Server Error" }, { status: 500 });
  }
}
