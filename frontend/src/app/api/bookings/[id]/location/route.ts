...
export async function POST(
  ...
  try {
    ...
    if (!authUser) {
      console.log(`[BOOKING_LOOKUP] route=/api/bookings/[id]/location method=POST rawId=${rawId} cleanId=${cleanId} authenticated=false`);
    ...
    if (bookingRes.rows.length === 0) {
      console.log(`[BOOKING_LOOKUP] route=/api/bookings/[id]/location method=POST rawId=${rawId} cleanId=${cleanId} userId=${authUser.userId} role=${authUser.role} found=false`);
    ...
    console.log(`[BOOKING_LOOKUP] route=/api/bookings/[id]/location method=POST bookingId=${canonicalId} userId=${authUser.userId} role=${authUser.role} found=true provider=${isAssignedProvider} customer=${isAssignedCustomer}`);
